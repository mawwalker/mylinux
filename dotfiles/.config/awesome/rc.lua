...
run_once({ "picom" })
run_once({ "sh /home/dsm/.screenlayout/dua.sh" })
run_once({ "fcitx5" })
run_once({ "wmname LG3D" }) -- Fix for java applications
run_once({ '/usr/lib/polkit-kde-authentication-agent-1 && eval $(gnome-keyring-daemon -s --components=pkc11,secrets,ssh,gpg) ' }) -- Fix for java applications
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "r", function () awful.spawn(rofi_settings) end,
